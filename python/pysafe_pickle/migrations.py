"""Schema evolution decorators and migration registry for pysafe-pickle."""
...
def migrate(*, from_version: int, to_version: int, type_name: str | None = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        func._pysafe_pickle_migration = True
        func._pysafe_pickle_from_version = from_version
        func._pysafe_pickle_to_version = to_version
        # Backward compatibility attributes for existing tests/checks
...
def apply_migrations(type_name: str, state: dict, from_version: int, to_version: int) -> dict:
    ...
    for from_v, to_v, func in chain:
        ...
        current_state["__pysafe_pickle_version__"] = to_v
...
def get_schema_version(obj) -> int | None:
    ...
    if hasattr(obj, "__pysafe_pickle_version__"):
        return getattr(obj, "__pysafe_pickle_version__")
    if hasattr(obj, "__pygraph_version__"):
        return getattr(obj, "__pygraph_version__")
    if hasattr(obj, "__dataclass_fields__"):
        cls = type(obj)
        if hasattr(cls, "__pysafe_pickle_version__"):
            return getattr(cls, "__pysafe_pickle_version__")
...
def set_schema_version(obj, version: int) -> None:
    ...
    cls.__pysafe_pickle_version__ = version
